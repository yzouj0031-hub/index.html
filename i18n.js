(function () {
  ...
  const EXACT = {
    ...
    '在命牌和发言旁显示AI模型标签，方便录屏辨识':'Show AI model labels beside role cards and speeches.','真人参与对局时防作弊：其他玩家的身份、AI 内心思考、狼队密谈和夜间私密行动都不会出现在页面上（不是用匿名那样盖住，而是根本不渲染），开局后本开关锁死。对局结束自动解除，想回看完整过程用复盘导出。':'Anti-cheat for games with a human player: other players\u2019 roles, AI reasoning, wolf-pack chat and private night actions are never rendered into the page (not merely hidden like anonymous mode). The switch locks once the game starts and releases when it ends; use the replay export to review everything afterwards.','开启后模型标签显示完整模型名（如 Claude Opus 4.5、Gemini 3 Pro），关闭则只显示品牌简称（如 Claude、Gemini）。命牌上名字过长会截断，鼠标悬停可看完整模型 ID。':'Show the full model name on the badge (e.g. Claude Opus 4.5, Gemini 3 Pro) instead of just the brand (Claude, Gemini). Long names are truncated on role cards; hover to see the full model ID.','开启后AI会输出思维过程（更耗token但可观察推理）；关闭则直接输出发言和操作（省token，速度更快）':'Allow AI reasoning output; disabling saves tokens and is faster.',
    ...
    '精简特效':'Reduced effects','主持人':'Host','音效':'Sound','模型标签':'Model labels','严格模式':'Strict mode','完整模型名':'Full model name','思维链':'Reasoning','自由输出':'Natural output',
